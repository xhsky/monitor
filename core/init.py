...
import os, sys
import yaml, json
from module import client, db, config, logger, common
...
class init(object):
    ...
    def login(self, host_info_key):
        ...
        """
        error_host_conn_key="host_error"    # 列表(队列), 存储错误信息
        host_soft_info_key="host_soft_info" # 字典, 存储集群ip, 及其软件状态
        ...
        for hostname in host_dict.keys():
          if hostname not in exists_host_dict.keys():
                ...
                if stats==1:
                    ...
                    self.__db_client.hset(host_soft_info_key, hostname, soft_info)
                    normal_ip_list.append(hostname)
                else:
                    # 将有问题ip写入redis
                    self.__db_client.list_set(error_host_conn_key, stats)
          else:
